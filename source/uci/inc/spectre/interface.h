...
private:
  ...
  static constexpr std::string_view UCI_MODE = "uci";

  /// @brief Positive Response for UCI mode of operation.
  static constexpr std::string_view UCI_MODE_RESPONSE = "uciok";

  /// @brief Request to synchronize with GUI.
  static constexpr std::string_view IS_READY= "isready";

  /// @brief Positive Response to synchronize with GUI.
  static constexpr std::string_view IS_READY_RESPONSE = "readyok";

  /// @brief Generic log message command.
  static constexpr std::string_view UNKNOWN_CMD = "Unknown Command!";

  /// @brief Request to quit the program.
  static constexpr std::string_view QUIT_CMD = "quit";

  /// @brief Generic message to display when quitting the program.
  static constexpr std::string_view QUIT_MSG = "Quitting the program!!!";

  /// @brief flag to track the UCI mode of operation.
  bool uciFlag_ = false;

  /// @brief Get the command Request from UI.
  bool GetOption(std::string& str);
